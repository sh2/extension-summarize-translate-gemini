...
ユーザー操作を伴わない自動テストでは、次の 2 つの API が host access を要求する。

- `chrome.scripting.executeScript()`（fixture ページからの選択テキスト取得）
- `chrome.tabs.captureVisibleTab()`（抽出失敗時の fallback）
...
- `chrome.tabs.query({ active: true, currentWindow: true })` だけを横取りし、
  fixture tab の `{ id, url, title, active: true }` を返す。
...
| `e2e/helpers/extension-context.js` | 一時 extension コピー、persistent context 起動、service worker 取得、設定投入、tab ID 取得を行う helper を追加する。 |
...
| `findTabIdByUrl(worker, urlPattern)` | `chrome.tabs.query()` で tab ID を取得する |
...
- 引数が `{ active: true, currentWindow: true }` を含む場合に限り、fixture tab の
  `{ id, url, title, active: true }` 1 件を resolve する。
- それ以外の呼び出しは元の `chrome.tabs.query` へそのまま渡す。
- fixture tab の ID・URL・title は `addInitScript` の引数として渡し、script 内に
  ハードコードしない。
...
3. service worker から fixture tab の ID を取得する。
...
| popup に汎用 error が表示される | tabs.query stub の注入タイミング（navigation 前か）、fixture tab ID、コピー manifest の host permissions | stub の対象を scripting や storage まで広げること |
