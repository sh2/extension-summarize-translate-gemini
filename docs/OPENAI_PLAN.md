...
モデルバージョンの表示条件は `languageModel` の値に `/` が含まれるかどうかで判定する（複数モデルのフォールバックチェーン時に実際に使われたモデルを確認するため）。

OpenAI では UI 上 `zz` のみが表示されるため `/` を含まず、単一モデル指定であるためモデルバージョンは表示されない。`utils.js` の `generateContentOpenAI` / `streamGenerateContentOpenAI` でも OpenAI 応答から `modelVersion` への正規化は行わない。

---

## Phase 5: `popup.js` — 最小限の変更 ✅ 実装済み
...
## Phase 6: `templates.html` — 変更不要 ✅ 完了
...
| Phase 5 | `popup.js` 最小限の調整 | ✅ 完了 |
| Phase 6 | `templates.html` 確認 | ✅ 完了 |
| i18n | メッセージキー追加 | ✅ 完了 |
