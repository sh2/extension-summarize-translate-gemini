...
本書は、11カード再編と save-bar / scrollspy / host permission 分岐を導入した**初期実装計画の記録**である。現行実装のうち、以下 2 点は後続の [PLAN_OPTIONS_UI_FIXES.md](./PLAN_OPTIONS_UI_FIXES.md) で変更されており、本書では**歴史的記録**として扱う。

1. provider カードの `is-inactive-provider` による「非選択側の弱表示」
2. `API Provider` / `Default Actions` / `Behavior` の `fieldset > legend` 維持前提と、それに依存する静的テスト期待値

したがって、本書中の `is-inactive-provider` 弱表示や `fieldset > legend` を前提とする記述は、初期案の記録として読むこと。現行の正とする仕様は [PLAN_OPTIONS_UI_FIXES.md](./PLAN_OPTIONS_UI_FIXES.md) と実装ファイルを優先する。
...
| プロバイダ別カード | **初期方針**として `Gemini API` / `OpenAI-compatible API` は常時表示し、非選択側は弱く表示する。**現行実装**では常時表示は維持しつつ、選択状態の視覚差は `Current provider` バッジのみ |
...
- `Gemini API` と `OpenAI-compatible API` のカードは provider 切り替え時も常時表示する。**初期方針では**非選択側に `is-inactive-provider` のような状態クラスを付与して弱表示し、サイドバー、アンカージャンプ、スクロールスパイとの整合を保つ想定だった。**現行実装では**常時表示は維持しつつ、選択状態の視覚差は `Current provider` バッジの表示有無だけで表現する。
...
| `extension/_locales/*/messages.json` | セクション見出し / サイドバー、初期化失敗メッセージ、host permission API 失敗メッセージ用の新規 i18n キーを追加（後述）。一般的な select 値・placeholder は英語の静的テキストとし、i18n キーを設けない |
...
- **初期方針では** API Provider、Default Actions (No Selection)、Default Actions (Selection) の各ラジオ群を `<fieldset>` と `<legend>` で囲み、Behavior の5チェックボックスも `<fieldset>` / `<legend>` でグループ化する想定だった。**現行実装では**右ペインの重複表示を避けるため `legend` は削除し、`fieldset` のみを残している。この段落は初期案の記録として保持する。
...
- **初期方針では**非選択 provider カードを背景・境界線・任意の状態ラベル（例: `Current provider`）で弱表示し、テキストや入力欄のコントラストを維持する想定だった。**現行実装では**この弱表示は廃止し、`Current provider` バッジだけで選択状態を示す。
...
> **実装後の変更（2026-07-20）**: 以下で「一般的な選択肢・placeholder 用の新規キー」としていた 8 キーは、全言語で英語表示とする方針に変更した。このためキーは全ロケールの `messages.json` から削除済みであり、`extension/options.html` に英語の静的テキストとして定義している。以降の当該キーを追加・`data-i18n` で適用する記述は、初期計画時点の内容である。
...
- ~~一般的な選択肢・placeholder 用の新規キー:~~ **方針変更により未導入・削除済み**
...
- theme / font-size の選択肢、OpenAI reasoning の `Unspecified` option、custom action label の placeholder は、`extension/options.html` に英語の静的テキストとして定義し、ローカライズしない。したがって、これらには `data-i18n` / `data-i18n-placeholder` を付与しない。`templates.html` に定義されている select 要素の option / optgroup 文言（`Auto-fallback` / `Stable` / `Preview` / `Gemma` / `User-specified`、各言語名）も翻訳対象に含めず、テンプレートに静的定義された表記のまま維持する。特に `User-specified` は翻訳せず英語のままとする。OpenAI reasoning の `Specified` optgroup も翻訳しない。モデル名、モデルの thinking 表現、custom action の具体的な英語プロンプト例は技術的な値・例示として原文を維持する
- すべての `extension/_locales/*/messages.json`（en を含む15ロケール）には、section title 8件、状態ラベル1件、初期化失敗1件、host permission API 失敗1件を追加する。上記の一般的な選択肢・placeholder 8件は追加せず、既存の定義も削除する。`templates.html` に定義されている select 要素の表示文言は静的定義のままとし、default locale へのフォールバックにも依存しない
...
- 全 input / select / textarea（`templates.html` から生成する `languageModel` / `languageCode` を除く）が、対応する `label[for]` を1個だけ持つこと。`aria-describedby` を指定したコントロールは、参照先の補足要素が存在すること。**初期案では**ラジオ群と Behavior に `fieldset > legend` を要求していたが、**現行実装では** `legend` は削除済みで、この期待値は後続修正で取り下げられている。
...
検証項目（1つの `describe`・最小限の `it`）。`is-inactive-provider` は**初期案では**「選択中でない provider 側にだけ付く」点を明示的に検証し、両方付与 / 両方未付与の回帰も漏れなく拾う想定だった。**現行実装では** class 付与自体は内部状態として残りうるが、視覚的な弱表示は後続修正で廃止されている:
...
| プロバイダ切り替え | **初期方針**では `#geminiSection` / `#openaiSection` は常時表示とし、非選択側のみ弱表示することで、サイドバー・アンカージャンプ・スクロールスパイとの不整合を避ける想定だった。**現行実装**では常時表示は維持しつつ、視覚差は `Current provider` バッジのみ |
...
| 翻訳 | section title キー8件、状態ラベル1件、初期化失敗1件、host permission API 失敗1件（計11件）を en を含む15ロケール全てに追加する。theme / font-size、OpenAI reasoning の `Unspecified`、custom action label の placeholder は `options.html` の英語静的テキストとし、ロケールキーを設けない。`templates.html` に定義されている select 要素の option / optgroup 文言も翻訳対象に含めず静的定義のまま維持し、`User-specified` は英語のまま、OpenAI reasoning の `Specified` optgroup も非翻訳とする。`extension-integrity.test.js` のキー一致検証に抵触しないよう、ロケールキーを変更する場合は同一コミットで全ロケールを更新する。説明文キーと見出しキーの分離により、将来の文言調整やレイアウト変更時の再利用性が高まる |
...
6. **_locales**: en を含む15ロケールに `options_section_*`、`options_provider_selected`、初期化失敗キー、host permission API 失敗キーを同一コミットで一括追加する。theme / font-size の選択肢、OpenAI reasoning の `Unspecified`、custom action label の placeholder は英語の静的テキストとし、ロケールキーを追加しない。`templates.html` に定義されている select 要素の option / optgroup 文言は翻訳せず静的定義のまま維持し、`User-specified` も英語のままとする。旧計画の不要キー案は採用しない
