...
### 1.1 Phase 4 の実装結果と Phase 4a への分離

Phase 4 の jsdom characterization test では、Markdown link の `data:text/html,...` は `href` を失う一方、
Markdown image の `data:text/html,...` は `img[src]` に残る現行挙動が確認された。この結果は
`test/dom/markdown.test.js` に観測結果として固定するが、危険 URL が残らないという M-05 の安全性要件は
現時点では満たさない。

この問題の production 修正、URL protocol allowlist、M-05 の安全性期待値への更新は、
[`TESTING_PHASE_4a.md`](./TESTING_PHASE_4a.md) に分離する。Phase 4 では `extension/utils.js` を変更せず、
Phase 4a の実装後に Phase 4 の M-05 安全性要件が満たされる。
...
#### M-05: SVG / data URL を含む代表 payload の現行挙動を記録する

`<svg onload="alert(1)">`、`[危険リンク](data:text/html,alert)`、`![危険画像](data:text/html,alert)`
などの代表 payload を渡す。

このケースでは、入力に直接含まれる SVG のような生 HTML は `textContent` により先にエスケープされる。
Markdown 構文から生成された `data:` URL は、link と image で sanitize 後の挙動が異なる可能性があるため、
Phase 4 ではその現行挙動を分けて記録する。`img[src]` に残る `data:` URL の安全化は
[`TESTING_PHASE_4a.md`](./TESTING_PHASE_4a.md) で実施する。

確認事項:

- raw SVG が live DOM 要素として生成されない。
- SVG の event handler attribute が live DOM に残らない。
- Markdown link の `data:` URL が `a[href]` に残らない現行挙動を確認する。
- Markdown image の `data:` URL が `img[src]` に残る現行挙動を確認する。
- Markdown 構文から生成された link / image は、要素の有無ではなく sanitize 後の属性と text / alt text を検証する。
...
以下をすべて満たしたとき、Phase 4 のテスト導入は完了とする。Markdown URL の安全化を含む
セキュリティ上の完了は、[`TESTING_PHASE_4a.md`](./TESTING_PHASE_4a.md) の完了後とする。

- `convertMarkdownToHtml()` を本番と同じ marked / DOMPurify build と jsdom で実行するテストがある。
- script、event handler、`javascript:` URL、SVG / data URL の代表 payload に対する現行挙動を検証している。
- `data:` image URL が残る既知の現行挙動を再現しており、その安全化を Phase 4a で実施することを記録している。
...
Phase 4 の characterization により確認された Markdown image の `data:` URL 問題は、
[`TESTING_PHASE_4a.md`](./TESTING_PHASE_4a.md) で先に安全化する。Phase 4a 完了後、個人向け計画の
次段階は最小 Chromium E2E である。そこでローカル mock API を使い、
